...
└── e_ego/
    └── e_c_corpus/
        └── ec_v_valetudo/               # code ecv
            └── ecv__/
                ├── ecv__.toml
                ├── ecv__log__weight.jsonl    # Annales weight series (keyed by YYYYMMDD)
...
**Hand-authored annotation is TOML; the record contract is JSON/JSONL** — machine-owned, though hand-editable at the source (I6). All TOML is parsed with `toml_edit` (format-preserving) so comments and ordering survive a rewrite by code or LLM (I8). There is **no global config file** — UI, mutation-confirm behavior (§7.3), the one-character char slot (§5.1), and per-core defaults (e.g. Album's `person`) are all hardcoded, so the tree is the only state. What is genuinely the *environment's* is read per command and stored nowhere — `$PANTHEON_ROOT` (§6.2), and the editor `edit` opens (`$VISUAL`/`$EDITOR`, §7.3); neither is Pantheon's to configure. Two hand-written TOML surfaces remain, both annotation, neither behavioral:
