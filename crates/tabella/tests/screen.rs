//! Tabella's **real** screen, driven (P§2, P§3).
//!
//! Not a fake app and not a pty. [`porticus::drive`] runs the same `handle` the loop
//! runs, over the same lineup `tab`'s bare short opens, with the same in-process
...
//! integration test links the *lib*, so Tabella's `App` was not nameable from a test
//! and none of this could be written. The chrome was the only thing a test could
//! drive; this is the other half — Tabella's own lineup, its folds, and the
...
use tabella::TabellaApp;

static COUNTER: AtomicU32 = AtomicU32::new(0);

fn fresh_root() -> PathBuf {
    let n = COUNTER.fetch_add(1, Ordering::Relaxed);
    let dir = std::env::temp_dir().join(format!("tab-screen-{}-{n}", std::process::id()));
...
/// Set the tree up through the real binary, so the screen reads what `tab` wrote —
/// the same crossing a hand makes (I8).
fn tab(root: &Path, args: &[&str]) -> i32 {
    Command::new(env!("CARGO_BIN_EXE_tab"))
        .arg("-C")
        .arg(root)
        .args(args)
        .output()
...
fn listed(root: &Path) -> Vec<String> {
    let out = Command::new(env!("CARGO_BIN_EXE_tab"))
        .arg("-C")
        .arg(root)
        .args(["list", "-H", "ecv"])
        .output()
...
fn seeded() -> PathBuf {
    let root = fresh_root();
    assert_eq!(
        tab(&root, &["ecv", "trip_idea", "A thought."]),
        0,
...
/// The node's records are on screen, folded by Tabella's own fold.
#[test]
fn the_screen_draws_the_nodes_records() {
    let root = seeded();
    let frame = porticus::drive(
        &mut TabellaApp::new(&root),
        &root,
        &porticus::keys(""),
        90,
        18,
    ...
    .unwrap();
    assert!(
        frame.contains("T A B E L L A"),
        "the name-word is the header (P§8): {frame}"
    );
    assert!(frame.contains("trip_idea"), "{frame}");
}

/// A lineup is checked before a terminal is ever taken (P§3), and Tabella's was only
...
fn the_lineup_is_legal_and_named() {
    ...
    for view in ["records", "read", "insights"] {
        assert!(frame.contains(view), "the tab strip names {view}: {frame}");
    }
}

/// An empty node keeps its chrome and says so calmly (I7, P§4) — in Tabella's own
...
fn an_empty_node_says_so_calmly() {
    ...
    .unwrap();
    assert!(frame.contains("no documents here"), "{frame}");
}
...
/// `Action::Remove` on the focused row, Tabella's `on_action` builds `tab rm …`,
