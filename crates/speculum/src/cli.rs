//! `spe` — Speculum, the review lens: every core folded across any horizon (§12, P§9).
//!
...
//! It may *relay* a human-initiated write — you fix a balance, drop a stale reading —
//! by shelling out to the same verb a hand would type (§7.2), never by linking a core's
//! lib (I5).
//!
...
/// The cores Speculum reaches. Discovered, never required: a tile whose core isn't
...
pub(crate) const RATIONES: &str = "rat";
pub(crate) const FASTI: &str = "fas";
...
#[command(
    name = "spe",
    version,
    about = "Speculum — the review lens (the mirror)",
...
/// Run `spe` exactly as the binary runs it (§7.3) — parse `argv`, dispatch, and return
...
fn figures(root: &std::path::Path) -> Value {
    json!({
        ...
        "holdings": count(root, RATIONES, &["list"]),
        "placements": count(root, FASTI, &["list"]),
...
fn help_json() -> Value {
    json!({
        "name": "speculum",
        "short": "spe",
        "about": "the review lens — every core folded across any horizon",
