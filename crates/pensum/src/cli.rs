//! Pensum's CLI (§7). stdout is JSON when piped, a table on a TTY (§7.3).
//!
//! **This is the lib's, not the bin's** — `main.rs` is a five-line shell over
//! [`run_cli`]. The reason is testability, not taste: an integration test links the
//! lib, so a `Cli` and a screen in the bin are a `Cli` and a screen no test can reach
//! (§14, P§3). Everything here is `pub(crate)` bar `run_cli` itself, so no verb becomes
//! callable as a Rust function and the JSON stays the only contract (I4).
//!
//! The CLI owns only what is Pensum's own: its positionals (`[home] <name> [text]`)
...
/// Run `pen` exactly as the binary runs it (§7.3) — parse `argv`, dispatch, and return
