//! Album's **real** screen, driven (P§2, P§3).
//!
//! Not a fake app and not a pty. [`porticus::drive`] runs the same `handle` the loop
//! runs, over the same lineup `alb`'s bare short opens, with the same in-process
...
//! integration test links the *lib*, so Album's `App` was not nameable from a test
//! and none of this could be written. The chrome was the only thing a test could
//! drive; this is the other half — Album's own lineup, its folds, and the
...
fn fresh_root() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("alb-screen-{}-{n}", std::process::id()));
...
/// Set the tree up through the real binary, so the screen reads what `alb` wrote —
/// the same crossing a hand makes (I8).
fn alb(root: &Path, args: &[&str]) -> i32 {
...
fn seeded() -> PathBuf {
    let root = fresh_root();
    assert_eq!(
        alb(&root, &["csa", "John Appleseed", "--closeness", "close"]),
        0,
...
/// The node's records are on screen, folded by Album's own fold.
#[test]
fn the_screen_draws_the_nodes_records() {
    ...
    .unwrap();
    assert!(
        frame.contains("A L B U M"),
        "the name-word is the header (P§8): {frame}"
    );
    assert!(frame.contains("john_appleseed"), "{frame}");
}

/// A lineup is checked before a terminal is ever taken (P§3), and Album's was only
...
/// An empty node keeps its chrome and says so calmly (I7, P§4) — in Album's own
...
fn an_empty_node_says_so_calmly() {
    ...
    .unwrap();
    assert!(frame.contains("nobody filed here"), "{frame}");
}
...
/// `Action::Remove` on the focused row, Album's `on_action` builds `alb rm …`,
