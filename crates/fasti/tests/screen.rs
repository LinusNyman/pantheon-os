//! Fasti's **real** screen, driven (P§2, P§3).
//!
//! Not a fake app and not a pty. [`porticus::drive`] runs the same `handle` the loop
//! runs, over the same lineup `fas`'s bare short opens, with the same in-process
...
//! integration test links the *lib*, so Fasti's `App` was not nameable from a test
//! and none of this could be written. The chrome was the only thing a test could
//! drive; this is the other half — Fasti's own lineup, its folds, and the
...
fn fresh_root() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("fas-screen-{}-{n}", std::process::id()));
...
/// Set the tree up through the real binary, so the screen reads what `fas` wrote —
/// the same crossing a hand makes (I8).
fn fas(root: &Path, args: &[&str]) -> i32 {
    Command::new(env!("CARGO_BIN_EXE_fas"))
        .arg("-C")
        .arg(root)
        .args(args)
        .output()
...
fn listed(root: &Path) -> Vec<String> {
    let out = Command::new(env!("CARGO_BIN_EXE_fas"))
        .arg("-C")
        .arg(root)
        .args(["list", "-H", "aof"])
        .output()
...
/// The node's records are on screen, folded by Fasti's own fold.
#[test]
fn the_screen_draws_the_nodes_records() {
    let root = seeded();
    let frame = porticus::drive(
        &mut FastiApp::new(&root),
        &root,
        &porticus::keys(""),
        90,
        18,
    ...
    .unwrap();
    assert!(
        frame.contains("F A S T I"),
        "the name-word is the header (P§8): {frame}"
    );
    assert!(frame.contains("mvp_phase"), "{frame}");
}

/// A lineup is checked before a terminal is ever taken (P§3), and Fasti's was only
...
fn an_empty_node_says_so_calmly() {
    ...
    .unwrap();
    assert!(frame.contains("nothing placed here"), "{frame}");
}
...
/// `Action::Remove` on the focused row, Fasti's `on_action` builds `fas rm …`,
